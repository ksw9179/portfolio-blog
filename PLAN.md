...
- [ ] B 계정으로 A의 글 수정 시도 → 실패해야 함
- [ ] B 계정으로 A의 글 삭제 시도 → 실패해야 함
- [ ] 로그아웃 상태에서 글 작성 시도 → 실패해야 함
- [ ] B가 `author_id`를 A로 위조해서 작성 시도 → 실패해야 함
- [ ] 비공개 글이 로그아웃 상태 조회에 안 나옴
...
anime.js 사이트가 "고급져 보이는" 이유는 화려해서가 아니라 **절제 + 일관성** 때문이다.

- **다크 기본** (`#0a0a0b` 계열) + **단일 액센트 컬러** 1개
- **타이포그래피 중심** — 큰 제목, 넉넉한 여백, 폰트 2종 이하
  - 본문: Pretendard (한글) / 코드·라벨: JetBrains Mono
